...
// Blit rapido do Atari 2600. Ver comentario no header. O caro no writeLine era
// (a) uma chamada putPixel por pixel (~61 mil/quadro) com o swizzle recalculado
// em cada uma, e (b) o vga_rgb565to6() por pixel. Aqui pre-convertemos a paleta
// para bytes crus uma vez por quadro (256 ops) e escrevemos 4 pixels por store
// de 32 bits, com a ordem de bytes do swizzle igual a' do flushLine.
void VGA_Video::blitAtari2x(const uint8_t *buf, int w, int h, int stride,
                            const uint16_t *palette565)
{
  if (!vgaReady || !buf) return;
  if (w * 2 > VGA_XRES) w = VGA_XRES / 2;   // 160 -> 320

  // Paleta (indice 8 bits) -> byte cru do framebuffer, uma vez por quadro.
  ...
  for (int y = 0; y < h; y++) {
    int dy = y + yoff;
    if ((unsigned)dy >= VGA_YRES) break;
    const uint8_t *src = buf + y * stride;
    uint32_t *fb32 = (uint32_t *)(_fb + dy * VGA_XRES);
    // Cada palavra = 4 pixels logicos = 2 fonte dobrados.
    //   logico 4k+0,+1 = src[2k]   (a)      logico 4k+2,+3 = src[2k+1] (b)
    //   ordem de bytes (swizzle, igual flushLine): [+2,+3,+0,+1] = b,b,a,a
    for (int k = 0; k < words; k++) {
      uint32_t a = pal6[src[2*k]];
      uint32_t b = pal6[src[2*k + 1]];
