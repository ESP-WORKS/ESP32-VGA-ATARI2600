...
// Ponto de montagem do cartao (prefixo VFS). E' este firmware que decide o
// nome -- se teu bootloader/convencao usar outro (ex.: "/sd"), troque AQUI.
...
static int menu_rescan(void) {
  ...
  while ((de = readdir(dir)) != NULL && nbFiles < MENU_MAX_FILES) {
    ...
    if (de->d_type == DT_DIR) {
      ...
      // Varios drivers FATFS do ESP-IDF NAO preenchem d_type: vem
      // DT_UNKNOWN (0) para tudo. Sem este fallback, NENHUM arquivo entrava
      // na lista (o menu mostrava a pasta vazia mesmo com .bin dentro).
      // stat() no caminho completo resolve o tipo.
      char full[96];
      snprintf(full, sizeof(full), "%s/%s", romspath, de->d_name);
      struct stat st;
      if (stat(full, &st) != 0) continue;   // sumiu / erro de leitura: pula
      isDir = S_ISDIR(st.st_mode);
    }
    menu_addEntry(de->d_name, isDir);
  }
  closedir(dir);
...
void emu_init(void)
{
  ...
  // Fallback de MISO do SD. LilyGO/TTGO VGA32 usa GPIO2; ROBGO_RG e Olimex
  // usam GPIO35 (input-only, ideal para MISO). Tentamos 2 primeiro; se nao
  // montar, liberamos o barramento SPI e tentamos 35. Trocar o pino MISO exige
  // reinicializar o barramento -- por isso o spi_bus_free() entre as tentativas.
  // (O SPIN_NUM_MISO do iopins.h fica sem uso aqui: quem manda e' esta lista.)
  ...
  const int misoTry[2] = { 2, 35 };     // 2 = LilyGO ; 35 = ROBGO_RG / Olimex
  ret  = ESP_FAIL;
  card = NULL;

  for (int m = 0; m < 2 && ret != ESP_OK; m++) {
    spi_bus_config_t sd_bus_cfg;
    memset(&sd_bus_cfg, 0, sizeof(sd_bus_cfg));
    sd_bus_cfg.mosi_io_num     = SPIN_NUM_MOSI;
    sd_bus_cfg.miso_io_num     = misoTry[m];
    sd_bus_cfg.sclk_io_num     = SPIN_NUM_CLK;
    sd_bus_cfg.quadwp_io_num   = -1;
    sd_bus_cfg.quadhd_io_num   = -1;
    sd_bus_cfg.max_transfer_sz = 4000;

    esp_err_t bret = spi_bus_initialize(SDSPI_HOST_ID, &sd_bus_cfg, SPI_DMA_CH_AUTO);
    if (bret != ESP_OK && bret != ESP_ERR_INVALID_STATE) {
      printf("spi_bus_initialize (MISO=%d) falhou: %d\n", misoTry[m], (int)bret);
      continue;
    }

    // Poucas tentativas por pino (cartao em soquete as vezes precisa de 2-3).
    for (int t = 0; t < 3; t++) {
      ret = esp_vfs_fat_sdspi_mount(SD_MOUNT_POINT, &host, &slot_config, &mount_config, &card);
      if (ret == ESP_OK) break;
      printf("SD MISO=%d tentativa %d falhou (%d)\n", misoTry[m], t+1, (int)ret);
    ...
    if (ret == ESP_OK) {
      ...
      // device/host no erro; so' o barramento fica de pe' e precisa sair
      // para o proximo pino poder reconfigura-lo.
      spi_bus_free(SDSPI_HOST_ID);
    }
  }

  if (ret != ESP_OK) {
    printf("SD: nao montou em MISO 2 nem 35. Seguindo sem cartao.\n");
    card = NULL;
  }
...
// ---- Telemetria: tempo gasto so' no blit (emu_DrawScreen) -----------------
static int64_t s_drawAccumUs = 0;
static int     s_drawCount   = 0;

// Lido pelo go.cpp uma vez por segundo. Devolve o acumulado e zera.
extern "C" void emu_perfGetDraw(int64_t * us, int * count)
{
  *us = s_drawAccumUs; *count = s_drawCount;
  s_drawAccumUs = 0; s_drawCount = 0;
}
...
void emu_DrawScreen(unsigned char * VBuf, int width, int height, int stride)
{
  ...
  // Atari 2600: VBuf e' 160 x 192 (8 bits, indice na palette de 256 cores).
  // A tela VGA e' 320 x 240. O blitAtari2x dobra na horizontal (160 -> 320,
  // escala inteira, sem interpolacao), centra na vertical (offset 24) e escreve
  // direto no framebuffer em palavras de 32 bits com o swizzle embutido --
  // ~4x menos stores que o writeLine e sem conversao de cor por pixel.
  video.blitAtari2x(VBuf, width, height, stride, palette16);

  s_drawAccumUs += esp_timer_get_time() - _t0;
  s_drawCount++;
}
