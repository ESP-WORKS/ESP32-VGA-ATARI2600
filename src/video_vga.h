...
class VGA_Video
{
  public:
    ...
    // Blit rapido dedicado ao Atari 2600: 'buf' e' w x h, 8 bits (indice de
    // palette565). Dobra na horizontal (w -> 2w), centra na vertical e escreve
    // direto no framebuffer em palavras de 32 bits (4 pixels/store) com o
    // swizzle embutido. ~4x menos stores que writeLine e sem conversao de cor
    // por pixel (paleta pre-convertida). Usado pelo emu_DrawScreen.
    void blitAtari2x(const uint8_t *buf, int w, int h, int stride, const uint16_t *palette565);
